...
void BST_312 <ItemType>::insertItem(TreeNode*& t, const ItemType& newItem)
{
    ...
    if (t == NULL) {
        TreeNode *newTree = new TreeNode;       // There are not any nodes
        newTree->data = newItem;
        newTree->left = NULL;
        newTree->right = NULL;
        t = newTree;
    } else {
...
int BST_312 <ItemType>::countNodes(TreeNode* t) const
{
    ...
    int numNodes = 1;                               // node that you are on

    if(t->left != NULL){
        numNodes = numNodes + countNodes(t->left);  //Nodes on left side
    }

    if(t->right != NULL){
        numNodes = numNodes + countNodes(t->right); //Nodes on right side
    }
...
void BST_312 <ItemType>::postOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    ...
    // LRN
    postOrderTraversal(t->left, result);        //goes left until it finds null
    postOrderTraversal(t->right, result);       //goes right until finds null
    result.push_back(t->data);                  //pushes node as it makes it's way back to root
...
bool BST_312 <ItemType>::isItemInTree(const ItemType& item)
{
    ...
    while(tempRoot != NULL) {
        ...
        } else if (item > tempRoot->data) {             // if larger move down the right side
            tempRoot = tempRoot->right;
        } else {
            tempRoot = tempRoot->left;                  // if smaller move down the left to search
        }
    }
